...
def _accumulated_moments_for_inference(mean, variance, is_training):
  ...
  with tf.variable_scope("accu", values=[mean, variance], reuse=tf.AUTO_REUSE):
...
def linear(inputs, output_size, scope=None, stddev=0.02, bias_start=0.0,
  ...
  if inputs == None:
    return lambda x: linear(x, output_size, scope, stddev, bias_start, use_sn, use_bias)
  else:
...
def conv2d(inputs, output_dim, k_h, k_w, d_h, d_w, stddev=0.02, name="conv2d",
  ...
  if inputs == None:
    return lambda x: conv2d(x, output_dim, k_h, k_w, d_h, d_w, stddev, name, 
                            use_sn, use_bias)
  else:
...
def deconv2d(inputs, out_dim, k_h, k_w, d_h, d_w,
             stddev=0.02, name="deconv2d", use_sn=False):
  """Performs transposed 2D convolution of the input."""
  if inputs == None:
    return lambda x: deconv2d(x, out_dim, k_h, k_w, d_h, d_w,
                              stddev, name, use_sn)
  else:
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
      w = tf.get_variable("kernel", [k_h, k_w, out_dim, inputs.get_shape()[-1]],
        initializer=weight_initializer(stddev=stddev))
      if use_sn:
        w = spectral_norm(w)
      input_shape = inputs.get_shape().as_list()
      output_shape = tf.stack([tf.shape(inputs)[0], d_h*input_shape[1], d_w*input_shape[2], out_dim])
...
def batch_norm_v2(inputs, is_training, name):
  return tf.contrib.layers.batch_norm(inputs, decay=0.99, is_training=is_training, 
    center=True, scale=True, reuse=tf.AUTO_REUSE, scope=name)

def reconstruction_layer(inputs, g, num_searches, num_steps_per_search, batch_size, name='reconstruction_layer'):
  z_dim = g.noise_dim
  with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
    tiled_z = tf.get_variable('tiled_z', shape=[batch_size*num_searches, z_dim], dtype=tf.float32, 
      initializer=tf.random_normal_initializer(stddev=np.sqrt(1.0 / z_dim)), trainable=True)

  reconstruction_step = 0  
  input_shape = list(inputs.shape)
  tiled_image_batch = tf.cast(tf.reshape(
    tf.tile(tf.reshape(inputs, [batch_size, np.prod(input_shape[1:])]), [1, num_searches]), 
    [batch_size*num_searches] + input_shape[1:]), tf.float32)
  ...
  optimizer = tf.train.RMSPropOptimizer(learning_rate=1e-2, momentum=0.7)
  train_op = optimizer.minimize(total_loss, var_list=[tiled_z])

  def loop_cond(i, *args):
    return tf.less(i, num_steps_per_search)
  ...
  with tf.control_dependencies([tiled_z.initializer,
                                tf.variables_initializer(optimizer.variables())]):
    _, zs, losses = tf.while_loop(cond=lambda i, *args: tf.less(i, 200),
                               body=loop_body,
                               loop_vars=[reconstruction_step, tiled_z, per_image_loss], 
                               parallel_iterations=1, 
                               back_prop=True,
                               swap_memory=False)

    best_zs = tf.stack([
      zs[i + tf.argmin(losses[i:i+num_searches], axis=0)] 
        for i in range(0, batch_size*num_searches, num_searches)])
  
  reconstruction = g.callable_generator(best_zs, False)
  return reconstruction

def activation_layer(inputs, activation):
  if inputs == None:
    return lambda x: activation_layer(x, activation)
  else:
    return activation(inputs)

def cond_layer(inputs, operation, tf_bool_ph):
  if inputs == None:
    return lambda x: cond_layer(x, operation, tf_bool_ph)
  else:
    return tf.cond(tf_bool_ph, true_fn=lambda: operation(inputs), false_fn=lambda: inputs)

def my_cond_layer(inputs, operation, python_bool):
  if inputs == None:
    return lambda x: my_cond_layer(x, operation, python_bool)
  else:
    if python_bool:
...
def flatten_layer(inputs):
  if inputs == None:
    return lambda x: flatten_layer(x)
  else:
